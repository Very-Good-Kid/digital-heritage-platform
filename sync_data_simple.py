...
try:
    ...
    with sqlite_engine.connect() as sqlite_conn:
        result = sqlite_conn.execute(text("SELECT id, username, email, password_hash, is_admin, created_at FROM users"))
    ...
    try:
        with sqlite_engine.connect() as sqlite_conn:
            result = sqlite_conn.execute(text("SELECT id, name, description, icon, \"order\" FROM asset_categories"))
        ...
        with neon_engine.connect() as neon_conn:
            ...
            neon_conn.commit()
            print(f"   [OK] 资产分类数据同步完成")
    except Exception as e:
        print(f"   [WARN] 资产分类表不存在或同步失败: {e}")
    ...
    try:
        ...
        with neon_engine.connect() as neon_conn:
            ...
            neon_conn.commit()
            print(f"   [OK] 平台政策数据同步完成")
    except Exception as e:
        print(f"   [WARN] 平台政策表不存在或同步失败: {e}")

    print("\n" + "=" * 60)
    print("[SUCCESS] 数据同步完成!")
    print("=" * 60)
